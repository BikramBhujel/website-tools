...
def client_ip(request: Request) -> str:
    # Only trust X-Forwarded-For when the gateway is behind your own reverse proxy.
...
def enforce_rate_limit(request: Request) -> None:
    now = time.monotonic()
    bucket = _hits[client_ip(request)]
...
def check_count(files: list[UploadFile]) -> None:
    ...
    if len(files) > MAX_FILES:
        raise HTTPException(400, f"Maximum {MAX_FILES} files per operation.")
    for upload in files:
...
async def call_engine(
    ...
    output_name: str = "bbpdf-result.pdf",
) -> Response:
    multipart = []
    for upload in files:
        await upload.seek(0)
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            async with client.stream("POST", f"{ENGINE_URL}{endpoint}", data=fields or {}, files=multipart) as upstream:
                ...
                headers = {
                    "Content-Disposition": f'attachment; filename="{normalize_filename(output_name, "bbpdf-result.pdf")}"',
        ...
        raise HTTPException(504, "PDF processing timed out.") from exc
    except httpx.HTTPError as exc:
        raise HTTPException(502, "PDF engine is temporarily unavailable.") from exc


@app.get("/health")
async def health() -> dict[str, str]:
    return {"status": "ok", "service": "bbpdf"}
...
async def merge(request: Request, files: list[UploadFile] = File(...)) -> Response:
    ...
    check_count(files)
    if len(files) < 2:
        raise HTTPException(400, "Select at least two PDF files to merge.")
...
async def split(
    ...
    page_numbers: str = Form("all"),
) -> Response:
    enforce_rate_limit(request)
    check_count([file])
    pages = page_numbers.strip() or "all"
    if len(pages) > 1000 or any(c not in "0123456789,- all" for c in pages.lower()):
        raise HTTPException(400, "Invalid page selection.")
    return await call_engine(
...
async def compress(
    request: Request,
    file: UploadFile = File(...),
    optimize_level: int = Form(3),
) -> Response:
    enforce_rate_limit(request)
    check_count([file])
    if optimize_level < 1 or optimize_level > 5:
        raise HTTPException(400, "Compression level must be between 1 and 5.")
    return await call_engine(
        "/api/v1/misc/compress-pdf",
        [file],
        {"optimizeLevel": str(optimize_level)},
        "bbpdf-compressed.pdf",
    )
